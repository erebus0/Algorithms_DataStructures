...
void delete_edge(vector< vector< pair<int, int> > > &g, int n1, int n2, int w)
{
	g[n1].erase(find(g[n1].begin(), g[n1].end(), make_pair(n2, w)));
	g[n2].erase(find(g[n2].begin(), g[n2].end(), make_pair(n1, w)));
}
